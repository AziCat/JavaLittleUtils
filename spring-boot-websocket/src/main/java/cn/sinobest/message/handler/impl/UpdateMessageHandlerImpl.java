 ...
 * @date 2017.09.13
 */
public class UpdateMessageHandlerImpl implements IMessageHandler {
    private Log logger = LogFactory.getLog(UpdateMessageHandlerImpl.class);
    @Override
    public SocketMessage handleMessage(ClientMessage clientMessage){
        ...
        switch (status){
            case MessageType.STOPSEND://停止推送
                stopSend(clientMessage, socketMessage);
                break;
            default:
    ...
    private void stopSend(ClientMessage clientMessage, SocketMessage socketMessage) {
        List idList = (List) clientMessage.getParams().get("ids");
        if (null == idList || idList.size() == 0){
            String errorMsg = "params传入错误！";
            setErrorMessage(socketMessage,errorMsg);
        }else {
            logger.info("停止推送-->"+idList);
            JdbcService jdbcService = JdbcService.getInstance();
            StringBuilder sql = new StringBuilder();
            int[] types = new int[idList.size()];
            sql.append("UPDATE B_MESSAGE_CENTER T SET T.STOPSENDFLAG = '1',T.LASTUPDATETIME = SYSDATE WHERE T.SYSTEMID IN (");
            ...
            setSuccessMessage(socketMessage,"停止推送操作成功！");
